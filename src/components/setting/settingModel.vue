...
<template>
    <div class="settingModel">
        ...
        <p class="operatorImg" v-if="data.roleId ===2 || data.roleId=== 3">
            <span>运营者身份证扫描件：</span>
            <img :src="data.idcardBack" alt="运营者身份证扫描件">
            <i class="iconfont icon-gou"></i>
            <span class="idmask">身份证正反面扫描件已审核通过</span>
        </p>
        <p class="operatorImg" v-if="data.roleId ===4 || data.roleId=== 5">
            <span>管理员身份证扫描件：</span>
            <img :src="data.idcardBack" alt="管理员身份证扫描件背部">
            <i class="iconfont icon-gou"></i>
            <span class="idmask">身份证正反面扫描件已审核通过</span>
        </p>
        <p class="otherLink">
            <span>专栏主页：</span>
            <a target="_blank" :href="data.kgAuthorIndexUrl">{{data.kgAuthorIndexUrl}}</a>
        </p>
        <p >
            <span>其他资质：</span>
